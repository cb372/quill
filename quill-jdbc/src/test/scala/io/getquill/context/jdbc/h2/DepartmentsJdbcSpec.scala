...
class DepartmentsJdbcSpec extends DepartmentsSpec {
  ...
  override def beforeAll = {
    testContext.transaction {
      ...
      testContext.run(liftBatch(departmentEntries).foreach(p => departmentInsert(p)))
      testContext.run(liftBatch(employeeEntries).foreach(p => employeeInsert(p)))
      testContext.run(liftBatch(taskEntries).foreach(p => taskInsert(p)))
    }
    ()
  }
